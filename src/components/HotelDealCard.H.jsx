...
// Version H — "Cinematic Two-Strip"
// Based on F. Two horizontal strips on the image: top strip has name + stars + rating,
// bottom strip has price + savings + amenity icons. Middle area is open image.
// Color rule: black text on solid green/orange bg, green text on green/10 bg, orange text on orange/10 bg.
...
export default function HotelDealCardH({ deal, onClick }) {
  ...
  return (
    <div
      ...
      style={{
        height: 300,
        background: 'radial-gradient(ellipse at 50% 30%, rgba(255,200,150,0.08), transparent 60%), linear-gradient(170deg, #2A3040, #1E2832, #1a1a1a)',
      }}
    >
      ...
      {/* ── Top strip: name + stars + rating ── */}
      <div className="relative" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 80%, transparent 100%)' }}>
        <div className="px-[14px] pt-[12px] pb-[16px]">
      ...
      {/* ── Bottom strip: amenities + price + savings ── */}
      <div className="relative" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.5) 70%, transparent 100%)' }}>
        <div className="px-[14px] pb-[12px] pt-[24px]">
          {/* Amenity icons + room pills */}
          <div className="flex items-center justify-between mb-[8px]">
            <div className="flex items-center">
              {(deal.amenities || []).slice(0, 4).map((a, i) => (
                <div key={i} className={`inline-flex items-center ${i > 0 ? 'ml-[6px]' : ''}`}>
                  <span className="text-white/40 mr-[4px]"><AmenityIcon name={a} size={10} /></span>
                  <span className="font-['Lato',sans-serif] text-[10px] text-white/50 leading-[1]">{a.replace('Free ', '')}</span>
                </div>
              ))}
            </div>
            <div className="inline-flex items-center bg-white/10 rounded-full px-[8px] py-[4px]">
              <span className="font-['Lato',sans-serif] text-[10px] text-white/60 leading-[1]">{deal.nights}N · {deal.room_type}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="inline-flex items-center bg-green rounded-full px-[10px] py-[4px]">
              <span className="font-bold text-[11px] text-black leading-[1]">Save ${totalSaved}</span>
            </div>
          </div>

          {/* Taxes note */}
          <span className="font-['Lato',sans-serif] text-[10px] text-orange leading-[1] mt-[4px] inline-block">${deal.taxes || 0} taxes &amp; fees incl.</span>
        </div>
      </div>
    </div>
  )
