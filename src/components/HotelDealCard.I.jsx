...
// Version I — "Cinematic Grid"
// Based on F. Full image card with a 2×2 info grid overlaid on the bottom half.
// Top-left: name+stars. Top-right: rating. Bottom-left: price. Bottom-right: savings+amenities.
// Color rule: black text on solid green bg, green text on green/10 bg, orange text for taxes.
...
export default function HotelDealCardI({ deal, onClick }) {
  ...
  return (
    <div
      className="relative rounded-[12px] overflow-hidden flex flex-col justify-end"
      onClick={onClick}
      style={{
        height: 320,
        background: 'radial-gradient(ellipse at 50% 25%, rgba(255,200,150,0.08), transparent 60%), linear-gradient(170deg, #2A3040, #1E2832, #1a1a1a)',
      }}
    >
      ...
      {/* Save badge — top right, floating */}
      <div className="absolute top-[12px] right-[12px] inline-flex items-center bg-green rounded-full px-[10px] py-[4px]">
        <span className="font-bold text-[11px] text-black leading-[1]">{pctOff}% off</span>
      </div>

      {/* Bottom overlay — 2×2 grid */}
      <div className="relative" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.65) 60%, transparent 100%)' }}>
        <div className="px-[14px] pb-[14px] pt-[40px]">
          {/* Row 1: name+stars | rating */}
          <div className="flex items-start justify-between mb-[10px]">
            <div className="flex-1 mr-[8px]">
              ...
              <div className="flex items-center mt-[4px]">
                ...
                <span className="font-['Lato',sans-serif] text-[10px] text-white/50 leading-[1]">{deal.chain} · {deal.location}</span>
              </div>
            </div>
            <div className="flex flex-col items-end">
              <div className="inline-flex items-center bg-green rounded-[6px] px-[8px] py-[4px]">
                <span className="font-bold text-[12px] text-black leading-[1]">{deal.rating}</span>
              </div>
              <span className="font-['Lato',sans-serif] text-[10px] text-white/40 leading-[1] mt-[4px]">{ratingText}</span>
            </div>
          </div>

          {/* Row 2: price | savings + amenities */}
          <div className="flex items-end justify-between">
            ...
            <div>
              ...
              <div className="flex items-center mt-[4px]">
                <span className="font-['Lato',sans-serif] text-[11px] text-white/30 line-through leading-[1] mr-[6px]">${deal.original_price}</span>
                <span className="font-['Lato',sans-serif] text-[10px] text-orange leading-[1]">{deal.nights}N · ${totalAxel} total</span>
              </div>
            </div>

            {/* Right: savings + amenity icons */}
            <div className="flex flex-col items-end">
              <div className="inline-flex items-center bg-green/10 rounded-full px-[8px] py-[4px] mb-[6px]">
                <span className="font-bold text-[11px] text-green leading-[1]">Save ${totalSaved}</span>
              </div>
              <div className="flex items-center">
                {(deal.amenities || []).slice(0, 4).map((a, i) => (
                  <div key={i} className={`inline-flex items-center justify-center bg-white/10 rounded-full px-[4px] py-[4px] ${i > 0 ? 'ml-[4px]' : ''}`}>
                    <span className="text-white/40"><AmenityIcon name={a} size={10} /></span>
                  </div>
                ))}
              </div>
            </div>
